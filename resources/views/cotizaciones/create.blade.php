...
@section('content')
    ...
    <form method="POST" action="{{ route('cotizaciones.store') }}" id="form_cotizacion">
        @csrf
        ...
        <div style="background:white;padding:1rem;border-radius:8px;box-shadow:0 1px 4px rgba(0,0,0,0.08);margin-bottom:1rem;">
            ...
            {{-- Tabla de items --}}
            <table style="width:100%;border-collapse:collapse;font-size:0.8rem;" id="tabla_items">
                ...
                <tbody id="items_body">
                    <tr id="sin_items">
                        <td colspan="7" style="padding:1rem;text-align:center;color:#8A93A2;">No hay productos agregados</td>
                    </tr>
                </tbody>
            </table>
        </div>
        ...
        <button type="submit" style="background:#378ADD;color:white;padding:0.5rem 1.5rem;border:none;border-radius:6px;cursor:pointer;font-size:0.88rem;font-weight:600;">Guardar Cotización</button>
    ...
    <script>
        ...
        function buscarProducto() {
            ...
            fetch(`{{ route('cotizaciones.buscar-producto') }}?busqueda=${busqueda}`)
                .then(res => res.json())
                .then(data => {
                    if (data.encontrado) {
                        agregarItem(data);
       ...
       // Formatear numero con puntos de miles
...
function calcularTotales() {
    ...
    document.querySelectorAll('#items_body tr[id^="item_"]').forEach(tr => {
        ...
        const cantidad = parseFloat(tr.querySelector(`[name="items[${idx}][cantidad]"]`).value) || 0;
        const precio   = parseFloat(tr.querySelector(`[name="items[${idx}][precio_unitario]"]`).value) || 0;
        const ivaText  = tr.cells[4].textContent.replace('%', '');
