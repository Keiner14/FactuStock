...
<style>
    ...
    .precio-bajo { border-color: #e74c3c !important; background: #fff5f5 !important; }
    .alerta-precio {
        background: #FCEBEB; border: 1px solid #fca5a5;
        color: #791F1F; padding: 0.6rem 0.9rem;
        border-radius: 8px; font-size: 0.82rem;
        margin-bottom: 1rem; display: none;
    }
</style>

<h2 style="color:#378ADD; margin-bottom:1rem;">Nueva Cotización</h2>
...
<form method="POST" action="{{ route('cotizaciones.store') }}" id="form_cotizacion" onsubmit="return validarFormulario()">
    @csrf
    ...
    <div style="background:#e8f4fd;border-radius:6px;padding:0.5rem 1rem;margin-bottom:1rem;font-size:0.82rem;color:#185FA5;font-weight:600;">
        📋 Cotización N° {{ str_pad($consecutivo, 4, '0', STR_PAD_LEFT) }}
    </div>
    ...
    <div style="background:white;padding:1rem;border-radius:8px;box-shadow:0 1px 4px rgba(0,0,0,0.08);margin-bottom:1rem;">
        ...
        <div style="display:flex;gap:0.75rem;align-items:center;margin-bottom:0.75rem;">
            ...
            <button type="button" onclick="buscarCliente()"
                    style="background:#378ADD;color:white;padding:0 1.2rem;height:38px;border:none;border-radius:7px;cursor:pointer;font-size:0.82rem;font-weight:600;margin-top:1.2rem;">
                Buscar
            </button>
        </div>
    ...
    <div style="background:white;padding:1rem;border-radius:8px;box-shadow:0 1px 4px rgba(0,0,0,0.08);margin-bottom:1rem;">
        ...
        <div id="alerta_precio" class="alerta-precio"></div>
        ...
        <table style="width:100%;border-collapse:collapse;font-size:0.8rem;">
            ...
            <tbody id="items_body">
                <tr id="sin_items">
                    <td colspan="8" style="padding:1.5rem;text-align:center;color:#8A93A2;">
    ...
    <button type="submit" style="background:#378ADD;color:white;padding:0.6rem 1.8rem;border:none;border-radius:8px;cursor:pointer;font-size:0.88rem;font-weight:600;">💾 Guardar Cotización</button>
    <a href="{{ route('cotizaciones.index') }}" style="margin-left:0.5rem;color:#8A93A2;font-size:0.82rem;text-decoration:none;">Cancelar</a>
...
<div class="modal-overlay" id="modalProducto">
    <div class="modal-box">
        ...
        <div class="modal-producto-info" id="modal_producto_info">
            ...
            <span id="modal_producto_costo"></span> &nbsp;|&nbsp;
            <span style="color:#8A93A2;">Stock: </span>
            <span id="modal_producto_stock"></span>
        </div>

        <div class="modal-producto-error" id="modal_producto_error">
        ...
        <div id="modal_precio_alerta" style="display:none;background:#FCEBEB;border:1px solid #fca5a5;color:#791F1F;padding:0.5rem 0.8rem;border-radius:7px;font-size:0.78rem;margin-top:-0.3rem;margin-bottom:0.5rem;">
            ⚠️ El precio es menor al costo promedio.
        </div>
...
<script>
    ...
    function buscarCliente() {
        ...
        if (!busqueda) return;
        fetch(`{{ route('cotizaciones.buscar-cliente') }}?busqueda=${busqueda}`)
            .then(res => res.json())
            .then(data => {
                if (data.encontrado) {
    ...
    function abrirModalProducto() {
        ...
        document.getElementById('modal_precio').classList.remove('precio-bajo');
    ...
    function buscarProductoModal() {
        ...
        if (!codigo) return;
        fetch(`{{ route('cotizaciones.buscar-producto') }}?busqueda=${codigo}`)
            .then(res => res.json())
            .then(data => {
                if (data.encontrado) {
                    ...
                    document.getElementById('modal_producto_stock').textContent = data.stock;
    ...
    function validarPrecioModal() {
        ...
        if (precio > 0 && precio < costo) {
            input.classList.add('precio-bajo');
            alerta.style.display = 'block';
        } else {
            input.classList.remove('precio-bajo');
    ...
    function agregarDesdeModal() {
        ...
        if (precio === 0) {
            alert('Debes ingresar un precio de venta.');
            return;
        }
    ...
    function recalcular(idx) {
        ...
        const sub = cantidad * precio;
        const precioInput = tr.querySelector(`[name="items[${idx}][precio_unitario]"]`);
        if (precio > 0 && precio < costo) {
            precioInput.style.borderColor = '#e74c3c';
            precioInput.style.background = '#fff5f5';
        } else {
            precioInput.style.borderColor = '#D5D9E0';
            precioInput.style.background = 'white';
        }
        document.getElementById(`sub_${idx}`).textContent = '$' + formatear(sub);
        calcularTotales();
    }
    ...
    function validarFormulario() {
        ...
        document.querySelectorAll('#items_body tr[id^="item_"]').forEach(tr => {
            ...
            if (precio === 0) errores.push(`"${nombre}": debe ingresar un precio.`);
            if (precio > 0 && precio < costo) errores.push(`"${nombre}": precio menor al costo.`);
        });
        if (errores.length > 0) {
            const alerta = document.getElementById('alerta_precio');
            alerta.innerHTML = '⚠️ ' + errores.join('<br>');
            alerta.style.display = 'block';
            window.scrollTo(0, 0);
            return false;
        }
