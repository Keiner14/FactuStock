...
<head>
    ...
    <style>
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        .numero-box { display: table; width: 100%; margin-bottom: 15px; }
        .numero-izq { display: table-cell; width: 60%; }
        ...
        .badge-facturada { background: #EAF3DE; color: #27500A; }
        .badge-cancelada { background: #FCEBEB; color: #791F1F; }
...
<body>
    ...
    <div class="numero-box">
        <div class="numero-izq">
            <span class="badge-numero">COT-{{ str_pad($cotizacion->consecutivo, 4, '0', STR_PAD_LEFT) }}</span>
            <span class="badge-estado badge-{{ $cotizacion->estado }}">{{ ucfirst($cotizacion->estado) }}</span>
            <div class="fecha">Fecha: {{ $cotizacion->created_at->format('d/m/Y h:i A') }}</div>
        </div>
    </div>
    ...
    <div class="cliente-box">
        ...
        <div class="cliente-grid">
            <div class="cliente-col"><span class="label">Nombre: </span><strong>{{ $cotizacion->cliente->nombre }}</strong></div>
            <div class="cliente-col"><span class="label">Cédula: </span><strong>{{ $cotizacion->cliente->cedula }}</strong></div>
        </div>
        <div class="cliente-grid" style="margin-top:3px;">
            <div class="cliente-col"><span class="label">Celular: </span><strong>{{ $cotizacion->cliente->celular }}</strong></div>
            <div class="cliente-col"><span class="label">Dirección: </span><strong>{{ $cotizacion->cliente->direccion }}</strong></div>
        </div>
    </div>
    ...
    <div class="totales-box">
        <div class="t-row">
            ...
            <div class="t-valor">${{ number_format($cotizacion->subtotal, 2, ',', '.') }}</div>
        </div>
        <div class="t-row">
            <div class="t-label">IVA:</div>
            <div class="t-valor">${{ number_format($cotizacion->total_iva, 2, ',', '.') }}</div>
        </div>
        <div class="t-total">
            <div class="t-label">TOTAL:</div>
            <div class="t-valor">${{ number_format($cotizacion->total, 2, ',', '.') }}</div>
        </div>
    </div>
